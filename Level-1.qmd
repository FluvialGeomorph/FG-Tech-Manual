...
**Multiple Surveys**   
To make LiDAR surveys collected before the `📁base event`(#units_of_analysis) directly comparable to the base event, the `🗺️flowline` from each previous survey must be `📊calibrated` to the base event. This adjusts any changes in flowline planform between survey events to be expressed in terms of the base event longitudinal stationing. 

* If multiple LiDAR surveys exist for a project study area, the `🗺️flowline_points` for any previous LiDAR survey must be calibrated using the `🗺️flowline_points` of the base event. 
* For example, if 2016 is the base event, when deriving the `🗺️flowline_points` for a LiDAR survey from 2010, the base event's `🗺️flowline_points` (2016) must be used for the `🛠️Flowline Points` tool's `📊calibration_points` parameter value. 
* Set the `📊point_id_field` and `📊measure_field` parameters to the fields in the base event's `🗺️flowline_points` feature class. 
* Set the `📊search_radius` parameter to the maximum distance between the `🗺️flowline` of the current survey and the base event `🗺️flowline`. 
