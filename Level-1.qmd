...
* The `🗺️calibration_points`, `📊point_id_field`, and `📊measure_field` parameters can be left blank when processing the `📁base event`.

**Multiple Surveys**   
To make LiDAR surveys collected before the `📁base event` directly comparable to the base event, the `🗺️flowline` from each previous survey must be `📊calibrated` to the base event. This adjusts any changes in flowline planform between survey events to be expressed in terms of the base event longitudinal stationing. 
