...
#### `Install R Packages`
This tool installs the R packages required by the FluvialGeomorph toolbox. This tool also sets several configuration settings. This ArcGIS script tool calls the R script [`FluvialGeomorph-toolbox/install/Install_FG_R.R`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/install/Install_FG_R.R). 

#### `R Session Info`
This tool provides the output of the current R session. This is helpful for troubleshooting R environment issues. This ArcGIS script tool calls the R script [`FluvialGeomorph-toolbox/install/session.R`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/install/session.R). 
...
#### `02 - Hydro DEM`
This tool burns a [`cutlines`](#cutlines) feature class into the supplied DEM and creates the [`dem_hydro`](hydro-modified-raster) in the output workspace. See the [`dem_hydro`](hydro-modified-raster) section for a description of the raster dataset created. This ArcGIS script tool calls the Python script [`FluvialGeomorph-toolbox/tools/_02_HydroDEM.py`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_02_HydroDEM.py)

```{r echo=FALSE, fig.cap="02 - Hydro DEM Tool."}
knitr::include_graphics("figures/toolbox/tool_Hydro_DEM.png")
```

#### `03 - Contributing Area`
This tool calculates the contributing area from an input DEM. This ArcGIS script tool calls the Python script [`FluvialGeomorph-toolbox/tools/_03_ContributingArea.py`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_03_ContributingArea.py). This Python script uses the [TauDEM](https://hydrology.usu.edu/taudem/taudem5/index.html) [D-Infinity Contributing Area tool](https://hydrology.usu.edu/taudem/taudem5/help53/DInfinityContributingArea.html) to calculate the specific catchment area (which is the contributing area per unit contour length using the multiple flow direction D-infinity approach).
...
This tool calculates D8 flow accumulation and flow direction rasters from an input DEM. This ArcGIS script tool calls the Python script [`FluvialGeomorph-toolbox/tools/_03a_ContributingAreaD8.py`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_03a_ContributingAreaD8.py). 
```{r echo=FALSE, fig.cap="03a - Contributing Area D8 Tool."}
knitr::include_graphics("figures/toolbox/tool_Contributing_Area_D8.png")
```

#### `04 - Stream Network`
This tool creates a synthetic flow network from a contributing area raster and a stream initiation threshold. The ArcGIS script tool calls the Python script [`FluvialGeomorph-toolbox/tools/_04_StreamNetwork.py`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_04_StreamNetwork.py). 
...
This tool converts a stream network to points and calculates the drainage area. This ArcGIS script tool calls the Python script [`FluvialGeomorph-toolbox/tools/_04a_StreamNetworkPoints.py`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_04a_StreamNetworkPoints.py). 
...
This tool calculates stream gradient (or slope) and sinuosity for a [`flowline_points`](#flowline-points) or [`stream_network`](#stream-network) feature class. This ArcGIS script tool calls the Python script [`FluvialGeomorph-toolbox/tools/_04b_Gradient.R`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_04b_Gradient.R). 
...
#### `04c - Watershed`
This tool calculates the watershed for each input point. This ArcGIS script tool calls the Python script  [`FluvialGeomorph-toolbox/tools/_04c_Watersheds.py`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_04c_Watersheds.py). 
...
This tool creates the Level 3 Dimensions. This ArcGIS script tool calls the Python script [`FluvialGeomorph/FluvialGeomorph-toolbox/tools/_15c_XSPlanform.R`](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox/blob/master/tools/_15c_XSPlanform.R) which calls the functions [`fluvgeo::planform_dimensions`](https://github.com/FluvialGeomorph/fluvgeo/blob/master/R/planform_dimensions.R) to calculate the Level dimensions and [`fluvgeo::xs_metric_ratios`](https://github.com/FluvialGeomorph/fluvgeo/blob/master/R/xs_metric_ratios.R) to calculate the metric ratios. 
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "dem") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "DEM Raster Pixel Values.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "cutlines") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Cutlines Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "dem_hydro") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Hydro DEM Raster Pixel Values.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "contributing_area") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Contributing Area Raster Pixel Values.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "stream_network") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Stream Network Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "stream_network_points") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Stream Network Points Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "watershed_points") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Watershed Points Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "watershed") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Watershed Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "flowline") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Flowline Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "flowline_points") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Flowline Points Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "detrend") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Hydro DEM Raster Pixel Values.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "channel_slope") %>%
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Channel Slope Raster Pixel Values.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "xs") %>% 
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Regular Cross Section Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "riffle_floodplain") %>% 
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Riffle Floodplain Riffle Channel Cross Section Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "xs_points") %>% 
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Cross Section Points Fields.")
```
...
```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "_dims_L1") %>% 
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Level 1 Cross Section Dimension Fields.")
```

```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "_dims_L2") %>% 
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Level 2 Cross Section Dimension Fields.")
```

```{r echo=FALSE}
xs_dd <- filter(dd, feature_class == "_dims_L3") %>% 
         select(field_name:level)
kable(xs_dd, col.names = dd_col_names,
      caption = "Level 3 Cross Section Dimension Fields.")
```
