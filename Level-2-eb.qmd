...
* Ensure that riffle cross sections are digitized to the full width of the active floodplain. Edit each `🗺️ riffle_floodplain` feature class to ensure that it extends at least to the edge of the `🗺️ floodplain_polygon` layer, but no further. 
* This ensures that each `🗺️ riffle_floodplain` feature covers the entire floodplain, but does not extend too far into the uplands.  
...
* Use the ESRI `🛠️ Clip Raster` tool to clip the `watershed_contributing_area` raster to `🗺️ stream_network_buffer` to speed tool run time. 
