...
The purpose of this step is to define the rough contributing area of the study area. The  contributing area polygon derived in this step is almost always larger than the `study_area` polygon, unless analyzing small headwater reaches or an entire headwater watershed. 
...
Resolution is a measure of the smallest feature that can be represented in a dataset. The purpose of this step is to determine if the pre-made DEM has sufficient resolution to represent the stream channel and floodplain features in the project study area. If the pre-made DEM does not have sufficient resolution, it will be necessary to acquire LiDAR data and create a high resolution DEM for the project study area.

### Verify that the DEM elevations are in feet
Elevations in the pre-made DEM may be in meters or feet. The purpose of this step is to verify that the DEM elevations are in feet. If the DEM elevations are in meters, it will be necessary to convert the DEM elevations to feet.

### Create a hillshade to improve visualization
Hillshades are a visualization technique that simulates the appearance of sunlight on a surface. The purpose of this step is to create a hillshade from the pre-made DEM to improve visualization of the terrain features in the project study area.
...
LiDAR point cloud data is the raw data collected by LiDAR sensors. The purpose of this step is to acquire the LiDAR point cloud data for the project study area. This data will be used to create a high resolution DEM for the project study area.

### Build a DEM from LiDAR point cloud data
LiDAR point cloud data is a collection of points in 3D space that represent the surface of the terrain. The purpose of this step is to create a high resolution DEM from the LiDAR point cloud data. This DEM will be used to derive a detailed stream terrain model for the project study area.

### Verify that the DEM elevations are in feet
Elevations in the high resolution DEM may be in meters or feet. The purpose of this step is to verify that the DEM elevations are in feet. If the DEM elevations are in meters, it will be necessary to convert the DEM elevations to feet.

### Create a hillshade to improve visualization
Hillshades are a visualization technique that simulates the appearance of sunlight on a surface. The purpose of this step is to create a hillshade from the high resolution DEM to improve visualization of the terrain features in the project study area.
